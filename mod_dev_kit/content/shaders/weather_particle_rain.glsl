...
void main()
{
	...
	vec3 particle_world_position = graphics_offset + ((vertex_vec3_0_in + (mat3(mat_rotation) * (vertex_vec3_1_in * time_factor))) * particle_grid_size);
	...
	vec3 rotated_model_position = (mat_rotation * vec4(vertex_position_in, 0.0)).xyz;
	vec3 particle_view_position = (mat_view * vec4((particle_scale_factor * rotated_model_position) + particle_world_position, 1.0)).xyz;

	vec3 vertex_position_view = particle_scale_factor + particle_view_position;
