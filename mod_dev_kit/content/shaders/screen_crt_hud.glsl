...
void main()
{
	...
	// scanlines
	float scanline_brightness = sin(animation_time * 5.0 + warped_vertex_coord.y * 50.0) * 0.5 + 0.5;
	...
	float color_sample_a = texture(texture_color, coord_g + coord_noise_offset).a;

	vec3 backlight_color = vec3(0.02, 0.025, 0.04) * 5.0 * crt_effect_factor;
	...
	// combine effects
	color_sample = color_sample * object_color * screen_brightness;
	color_out = vec4(color_sample, color_sample_a);
	color_out += vec4(noise_sample) * (0.025 + 0.025 * scanline_brightness);
	...
	{
		color_out = mix(color_out, vec4(0.0, 0.0, 0.0, 1.0), 1.0 - pow(max(vignette_factor - 0.2, 0.0), 0.2));
	}
