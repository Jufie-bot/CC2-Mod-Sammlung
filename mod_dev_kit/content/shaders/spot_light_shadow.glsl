...
void main()
{
	...
	vec4 light_space_proj = mat_view_to_light_proj * vec4(view_position + (normal.xyz * 0.02), 1.0);
	light_space_proj.xyz /= light_space_proj.w;
	light_space_proj.xyz = light_space_proj.xyz * 0.5 + 0.5;
		...
		light_space_proj.z > 0.0 && light_space_proj.z < 1.0)
	{
		...
		if(surface_color.x + surface_color.y + surface_color.z > 0.0001)
		{
			...
			float shadow_sample = texture(texture_depth_shadow, light_space_proj.xyz);
			color_out *= shadow_sample;
		}
	}
}