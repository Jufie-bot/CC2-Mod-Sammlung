...
function render_flight_hud(screen_w, screen_h, is_render_center, vehicle)
	...
	local waypoints = vehicle:get_waypoint_path()
	for i=2, #waypoints do
		local waypoint_a_screen = update_world_to_screen(vec3(waypoints[i-1]:x(), 0, waypoints[i-1]:y()))
		local waypoint_b_screen = update_world_to_screen(vec3(waypoints[i]:x(), 0, waypoints[i]:y()))
		update_ui_line(waypoint_a_screen:x(), waypoint_a_screen:y(), waypoint_b_screen:x(), waypoint_b_screen:y(), color8(0, 255, 0, 100))
	end
end
