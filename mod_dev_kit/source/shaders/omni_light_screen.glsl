...
uniform sampler2D texture_light_color;

void main()
{
	//Sphere geometry is -0.5 to 0.5, so double the radius value
	light_color = textureLod(texture_light_color, vec2(0.5, 0.5), 16.0).xyz * 8.0;
	light_position_view = (mat_view * vec4(light_position, 1)).xyz;
	gl_Position = mat_proj * mat_view * vec4(light_position + (vertex_position_in * light_radius * 2.0), 1.0);
...
void main()
{
	...
	light_surface_magnitude *= 1.0 - clamp(light_to_fragment_length * light_radius_inv, 0.0, 1.0);
