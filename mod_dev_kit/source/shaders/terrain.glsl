...
void main()
{
	...
	vertex_normal_out = normalize((mat_view * vec4(vertex_normal_in, 0)).xyz);
...
void main() 
{
	...
	{
		gcolor_out = vec4(get_grass_color(vertex_position_out).rgb, ambient_sample);
	}
	else
	{
		gcolor_out = vec4(uni_cliff_color.rgb, ambient_sample);
	}
