        ...
        if selected_waypoint:get() then
            ...
            local window = ui:begin_window(update_get_loc(e_loc.upp_waypoint), 30, 30, screen_w - 60, screen_h - 60, atlas_icons.column_distance, true, 2)
                ...
                if get_is_vehicle_air(vehicle_definition_index) then
                    ...
                    local preset_altvalue = { 400, 1050, 1800 }
                    local preset_altbutton = ui:button_group({ "400", "1050", "1800" }, true)
                    if preset_altbutton >= 0 and preset_altbutton < #preset_altvalue then
                        selected_vehicle:set_waypoint_altitude(g_selection.waypoint_id, preset_altvalue[preset_altbutton + 1]) end
                end
            ui:end_window()
        else
            g_selection:clear()
