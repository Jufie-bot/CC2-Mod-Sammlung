...
function update(screen_w, screen_h, ticks) 
    ...
    if g_screen_index == 0 then
        ...
        if this_vehicle:get() then
            ...
            update_ui_image(screen_w/2 - 5, screen_h/2 - 5, atlas_icons.map_icon_circle_9, color_white, 0)

            local this_vehicle_dir = this_vehicle:get_direction()
			update_ui_line(screen_w/2, screen_h/2, screen_w/2 + (this_vehicle_dir:x() * 20), screen_h/2 + (this_vehicle_dir:y() * -20), color_white)
