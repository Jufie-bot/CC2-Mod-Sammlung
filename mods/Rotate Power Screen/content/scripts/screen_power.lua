...
function update(screen_w, screen_h, ticks) 
    ...
    ui:begin_window(update_get_loc(e_loc.upp_power), 5, 5, screen_w - 10, screen_h - 10, atlas_icons.column_power, true, 0)
        ...
        local spacing = 11
        local cx = 12
        local cy = (region_w - bar_w * #g_power_systems - spacing * (#g_power_systems - 2)) / 2 - 1

        for i = 1, #g_power_systems do
            local system = g_power_systems[i]

            render_power_bar(system.label, cy, cx, bar_h, bar_w, system.icon, system.desired, system.allocated)
            cx = cx + bar_w + spacing
        end
    ui:end_window()
...
function render_power_bar(label, x, y, w, h, icon, desired, allocated)
    update_ui_push_offset(x - 5, y)

    -- updates the text position
    -- 2nd parameter is height placement
    -- 4th/5th parameters are the allowed widths/heights of the text?
    -- last parameter is the rotation
    update_ui_text(5, h - 18, label, 200, 0, color_grey_dark, 4)
    
    update_ui_rectangle_outline(0, 0, w, h, color_white)

    local bar_h = math.max(math.floor((h - 4) * desired + 0.5), 1)
    
    -- 1st parameter is x position
    -- 2nd parameter is y position
    -- 3rd parameter is length of the bar
    -- 4th parameter is height of the bar

    local bar_w = math.max(math.floor((w - 4) * desired + 0.5), 1)
    
    -- left notch == top notch
    -- update_ui_rectangle(5, h - 2 - bar_h, 1, 3, color_white)
    update_ui_rectangle(2 + bar_w, -1, 1, 3, color_white)
    
    -- right notch == bottom notch
    -- update_ui_rectangle(w - 2, h - 2 - bar_h, 1, 3, color_white)
    update_ui_rectangle(2 + bar_w, h - 2, 1, 3, color_white)

    -- updates request power bar (original)
    -- bar_h = math.floor((h - 4) * desired + 0.5)
    -- update_ui_rectangle(2, h - 2 - bar_h, w - 4, bar_h, color_grey_dark)
    
    -- updates request power bar (mine)
    bar_w = math.floor((w - 4) * desired + 0.5)
    update_ui_rectangle(2, h - 6, bar_w, h - 4, color_grey_dark)
    
    -- updates current power bar (original)
    -- bar_h = math.floor((h - 4) * allocated + 0.5)
    -- update_ui_rectangle(2, h - 2, w - 4, bar_h, get_bar_color(desired, allocated))

    -- updates current power bar (mine)
    bar_w = math.floor((w - 4) * allocated + 0.5)
    update_ui_rectangle(2, h - 6, bar_w, h - 4, get_bar_color(desired, allocated))

    -- updates icons next to power bars
    update_ui_image(w + 4, h - 8, icon, get_bar_color(desired, allocated), 0)
