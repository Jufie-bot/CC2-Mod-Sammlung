...
-- Список островов с радарами
radar_islands = {
    {x = 500, z = 300, radius = 100},
    {x = 1200, z = 800, radius = 150},
    -- Добавляй новые острова здесь
}

-- Функция проверки блокировки сканирования
function is_scan_blocked(target_x, target_z)
    for i, radar in ipairs(radar_islands) do
        local dx = target_x - radar.x
        local dz = target_z - radar.z
...
function update(screen_w, screen_h, ticks) 
    ...
    if is_interference == false and is_damaged == false and is_powered then
        ...
        -- Получаем цели с проверкой на островные радары
...
--------------------------------------------------------------------------------
-- Функция формирования целей радара с проверкой на островные радары
--------------------------------------------------------------------------------
function get_radar_targets(range)
    ...
    for i = 0, vehicle_count - 1 do
        ...
        if vehicle:get() and vehicle:get_attached_parent_id() == 0 and vehicle:get_id() ~= screen_vehicle:get_id() then
            ...
            -- Блокировка по островным радарам
            if not is_scan_blocked(pos:x(), pos:y()) then
                ...
                if dist_sq < range_sq then
                    ...
                    if is_air or is_sea then
                        if vehicle:get_is_visible() and vehicle:get_is_observation_revealed() then
                            local icon_type = 0
                            if is_air then icon_type = 1 end
                            table.insert(targets, {
                                type = icon_type,
                                team = vehicle:get_team(),
                                pos = vec2(pos:x() - screen_pos:x(), pos:y() - screen_pos:y()),
                                dist = math.sqrt(vec2_dist_sq(pos, screen_pos)),
                                data = vehicle,
                            })
                        end
                    end
