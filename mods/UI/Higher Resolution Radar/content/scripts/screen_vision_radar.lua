...
function update(screen_w, screen_h, ticks) 
	...
	for i = 0, 24, 1 do
		...
		update_ui_line(screen_dir:x() * 0.97 * radius, -screen_dir:y() * 0.97 * radius, screen_dir:x() * radius, -screen_dir:y() * radius, radar_col(32))
	end

    if is_interference == false and is_damaged == false and is_powered then
        local screen_dir = screen_vehicle_map:get_direction()
        update_ui_line(0, 0, screen_dir:x() * 90, -screen_dir:y() * 90, radar_col(128))
    ...
    update_ui_image(205, screen_h - 15, atlas_icons.column_distance, col, 0)
