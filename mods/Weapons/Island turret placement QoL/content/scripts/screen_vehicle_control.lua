...
function render_selection_vehicle(screen_w, screen_h, vehicle)
    ...
    if screen_vehicle:get() then
        if screen_vehicle:get_team() == vehicle:get_team() then
            ...
            ui:begin_window(update_get_loc(e_loc.upp_status), 10, 10, left_w, 113, atlas_icons.column_pending, is_window_active, 2)
                ui:stat(update_get_loc(e_loc.hp), hitpoints .. "/" .. hitpoints_total, iff(damage_factor < 0.2, color_low, color_high))

                if def_index == e_game_object_type.chassis_land_turret then
                ...
                if def_index ~= e_game_object_type.chassis_carrier and def_index ~= e_game_object_type.chassis_land_robot_dog then
                    local is_vehicle_low_light = vehicle:get_is_low_light()
                    is_low_light, is_modified = ui:checkbox(update_get_loc(e_loc.low_light), is_vehicle_low_light)
                    if is_modified then vehicle:set_is_low_light(is_low_light) end
                end
            ...
            if #attachments > 0 and def_index ~= e_game_object_type.chassis_land_turret then
                local window = ui:begin_window(update_get_loc(e_loc.upp_ammo), 10, 141, left_w, { max=130 }, atlas_icons.column_stock, false, 2)
                local region_w, region_h = ui:get_region()
                local cy = 0
                ...
                if item ~= nil then
                    ...
                    local window = ui:begin_window(item.name .. "##queueitem", (screen_w - win_w) / 2, (screen_h - win_h) / 2, win_w, { max=win_h }, atlas_icons.column_pending, true, 2)
                        ...
                        if ui:button(update_get_loc(e_loc.upp_cancel_production), true, 1) then
                            cancel_turret(selected_island, g_command_center_ui.selected_facility_queue_item)
    ...
    if g_screen_index == 0 then
                ...
                if vehicle:get() then
                    ...
                    if vehicle_definition_index ~= e_game_object_type.chassis_spaceship and vehicle_definition_index ~= e_game_object_type.drydock then
                        ...
                        if is_visible and is_revealed then
                            ...
                            if vehicle_team == screen_team and vehicle:get_definition_index() ~= e_game_object_type.chassis_sea_barge then
        ...
        if g_selection:is_selection() then
            ...
            if is_placing_turret and g_highlighted.island_id > 0 and g_highlighted.turret_spawn_index ~= -1 then
                local island = update_get_tile_by_id(g_highlighted.island_id)

                if island:get() then
                    local marker_index, is_valid = island:get_turret_spawn(g_highlighted.turret_spawn_index)

                    local icon = get_icon_by_turret_type_id(g_command_center_ui.selected_item)
                    local text = update_get_loc(e_loc.upp_construct)

                    if not is_valid then
                        marker_progress = get_island_turret_progress(island, g_highlighted.turret_spawn_index)
                        if marker_progress >= 0 and marker_progress < 1 then
                            text = update_get_loc(e_loc.upp_cancel)
                            turret_item_id = get_island_turret_queue_number(island, marker_index)
                    ...
                    render_tooltip(10, 10, screen_w - 20, screen_h - 20, g_cursor_pos_x, g_cursor_pos_y, text_w + 24, 17, 10, function(w, h)
                        update_ui_image(2, 1, icon, iff(is_valid, color_status_ok, color_status_bad), 0)
...
function cancel_turret(island, turret_queue_index)
    island:set_facility_remove_production_queue_defense_item(turret_queue_index)
end
...
function get_island_turret_queue_number(island, marker_index)
    ...
    for i = 0, queue_count - 1 do
        local item_type_id, item_marker_index = island:get_facility_production_queue_defense_item(i)
        if item_marker_index == marker_index then
            return item_type_id, i
        end
    end
    return nil, -1
end
...
function get_island_turret_progress(island, turret_spawn_index)
    local marker_index, is_valid = island:get_turret_spawn(turret_spawn_index)
    local qtype_id, i = get_island_turret_queue_number(island, marker_index)
    if qtype_id ~= nil then
        if i == 0 then
            -- currently building
