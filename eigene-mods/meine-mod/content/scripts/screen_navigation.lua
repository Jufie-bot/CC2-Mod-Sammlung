...
function update(screen_w, screen_h, ticks)
    ...
    if g_dock_state_prev ~= nil and g_dock_state_prev == e_vehicle_dock_state.docked and
        g_dock_state_prev ~= this_vehicle:get_dock_state() then
        g_boot_counter = 30
        g_screen_index = 0
    end
    ...
    if g_screen_index == 0 then
        ...
        if this_vehicle:get() then
            ...
            if is_render_islands == false or g_is_island_names then
                for i = 0, island_count - 1, 1 do
                    ...
                    if island:get() then
                        ...
                        local screen_pos_x, screen_pos_y = get_screen_from_world(island_position:x(), island_position:y()
                            , g_camera_pos_x, g_camera_pos_y, g_camera_size, screen_w, screen_h)

                        if is_render_islands == false then
                            update_ui_image(screen_pos_x - 4, screen_pos_y - 4, atlas_icons.map_icon_island, island_color
                                , 0)
                        end
                        ...
                        if g_is_island_names and is_render_islands then
                            ...
                            local _, name_pos_y = get_screen_from_world(0, island_position:y() + island_size:y() * 0.5,
                                g_camera_pos_x, g_camera_pos_y, g_camera_size, screen_w, screen_h)

                            update_ui_text(screen_pos_x - 100, math.min(name_pos_y - 5, screen_pos_y - 14), island_name,
                                200, 1, island_color, 0)
                        end
                    end
                end
            end
                ...
                if vehicle:get() then
                    ...
                    if vehicle:get_is_visible() and vehicle:get_is_observation_revealed() then
                        if vehicle_attached_parent_id == 0 and i ~= this_vehicle_index then
                            ...
                            if vehicle_definition_index ~= e_game_object_type.chassis_spaceship and
                                vehicle_definition_index ~= e_game_object_type.drydock then

                                local vehicle_pos_xz = vehicle:get_position_xz()
                                local screen_pos_x, screen_pos_y = get_screen_from_world(vehicle_pos_xz:x(),
                                    vehicle_pos_xz:y(), g_camera_pos_x, g_camera_pos_y, g_camera_size, screen_w, screen_h)
                                ...
                                update_ui_image(screen_pos_x - icon_offset, screen_pos_y - icon_offset,
                                    region_vehicle_icon, element_color, 0)
                            end
                        end
                    end
                end
            ...
            update_ui_image(cx - 5, cy - 5, atlas_icons.map_icon_circle_9, color_white, 0)

            local this_vehicle_dir = this_vehicle:get_direction()
            update_ui_line(cx, cy, cx + (this_vehicle_dir:x() * 20), cy + (this_vehicle_dir:y() * -20), color_white)
    ...
    elseif g_screen_index == 1 then
        update_add_ui_interaction_special(update_get_loc(e_loc.interaction_navigate),
        ...
        local window = ui:begin_window(update_get_loc(e_loc.upp_map), 10, 10, screen_w - 20, screen_h - 20,
            atlas_icons.column_pending, true, 2)
    ...
    elseif g_screen_index == 2 then
        ...
        if this_vehicle:get() and this_vehicle:get_dock_state() == e_vehicle_dock_state.docked then
            local window = ui:begin_window(update_get_loc(e_loc.upp_carrier) .. "###launch", 10, 20, screen_w - 20,
                screen_h - 40, atlas_icons.column_pending, true, 2)
            ...
            ui:stat(atlas_icons.icon_health, string.format("%.0f/%.0f", hitpoints, total_hitpoints),
                get_factor_color(hitpoints / total_hitpoints))
            ui:stat(atlas_icons.column_weight, string.format("%.0f%%", inventory_factor * 100),
                iff(inventory_factor < 1, color_status_ok, color_status_bad))
