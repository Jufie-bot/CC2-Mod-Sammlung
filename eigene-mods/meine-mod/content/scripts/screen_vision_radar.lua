...
function update(screen_w, screen_h, ticks)
    ...
    update_add_ui_interaction_special(update_get_loc(e_loc.interaction_zoom_level),
        e_ui_interaction_special.gamepad_dpad_ud)
    ...
    local step = range / 4
    local border = 10
    local radius = math.min(screen_w, screen_h) / 2 - border
    ...
    -- local border = 20 -- Removed fixed border usage logic below
    ...
    if is_interference == false and is_damaged == false and is_powered then
        ...
        local angle = g_animation_time / 60 * math.pi * 2
        ...
        for i = 1, #targets do
            ...
            local angle_factor = clamp(remap(target_angle, 0, 2 * math.pi, 0, 1 + angle_fade_dist) - angle_fade_dist, 0,
                1)
            angle_factor = angle_factor ^ 3

            local target_color = iff(is_enemy, color8(255, 0, 0, math.ceil(angle_factor * 255)),
                radar_col(math.ceil(angle_factor * 255)))
            local target_color_off = iff(is_enemy, color8(255, 0, 0, math.ceil(angle_factor * 128)),
                radar_col(math.ceil(angle_factor * 128)))
            ...
            elseif type == 2 then
                ...
                update_ui_image(screen_pos:x() - 2, screen_pos:y() - 2, atlas_icons.screen_radar_missile,
                    iff(is_blink_on(5), target_color, target_color_off), 0)
            end
        end
    end
    ...
    update_ui_text(20, screen_h - 15, string.format("%.0f", range) .. update_get_loc(e_loc.upp_acronym_meters), 200, 0,
        col, 0)
    update_ui_image(75, screen_h - 15, atlas_icons.column_distance, col, 0)
    update_ui_text(0, screen_h - 15, string.format("%.0f", step) .. update_get_loc(e_loc.upp_acronym_meters),
        screen_w - 10, 2, col, 0)

    if is_damaged then
        render_status_label(10, screen_h / 2 - 9, screen_w - 20, 12, update_get_loc(e_loc.upp_damaged), color_status_bad
            , g_animation_time % 20 > 10, color_black)
    elseif is_powered == false then
        render_status_label(10, screen_h / 2 - 9, screen_w - 20, 12, update_get_loc(e_loc.upp_offline),
            color8(0, 16, 0, 255), g_animation_time % 20 > 10, color_black)
    elseif is_interference then
        render_status_label(10, screen_h / 2 - 9, screen_w - 20, 12, update_get_loc(e_loc.upp_interference),
            color_status_bad, g_animation_time % 20 > 10, color_black)
    elseif hostile_missile_dist < hostile_warning_distance then
        local blink_rate = math.floor(lerp(2, 30,
            math.max(0, hostile_missile_dist - 200) / (hostile_warning_distance - 200)) + 0.5)
        ...
        if hostile_missile_type == e_game_object_type.torpedo or hostile_missile_type == e_game_object_type.torpedo_decoy then
            render_status_label(10, 5, screen_w - 20, 12, update_get_loc(e_loc.upp_torpedo), color_status_bad,
                not g_is_warning_on, color_black)
        else
            render_status_label(10, 5, screen_w - 20, 12, update_get_loc(e_loc.upp_missile), color_status_bad,
                not g_is_warning_on, color_black)
        end
    end
end
...
function get_radar_targets(range)
    ...
    for i = 0, missile_count - 1 do
        ...
        if missile:get() and missile:get_is_visible() then
            ...
            if missile_type ~= e_game_object_type.torpedo_noisemaker and
                missile_type ~= e_game_object_type.torpedo_sonar_buoy then
                if dist_sq < range_sq then
                    table.insert(targets, {
                        type = 2,
