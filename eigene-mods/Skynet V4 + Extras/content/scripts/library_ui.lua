...
function rev_set_fow_island_scouted(island_id)
    if g_revolution_full_fow and g_scouted[island_id] == nil then
        ...
        -- Skynet V4 Hook: Insel-Entdeckung im Log vermerken
        if skynet_log ~= nil then
            local island = update_get_tile_by_id(island_id)
            if island and island:get() then
                skynet_log("Insel aufgeklaert: " .. island:get_name(), color_status_ok, atlas_icons.map_icon_island)
            end
        end
    end
end
